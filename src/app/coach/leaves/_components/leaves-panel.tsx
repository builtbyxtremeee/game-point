...
import { Trash2, Loader2, Plus, Calendar, AlertCircle } from 'lucide-react'
...
interface Props { leaves: Leave[]; batches: Batch[]; slots: Slot[] }
...
export function LeavesPanel({ leaves, batches, slots }: Props) {
  ...
  return (
    <div>
          ...
          <div className="card-body">
            <form action={formAction} className="form-grid">
              ...
              <div className="field-group form-full">
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                    <input type="radio" name="scope" value="court_wide" checked={scope === 'court_wide'} onChange={e => setScope(e.target.value)} /> Court-Wide
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                    <input type="radio" name="scope" value="batch" checked={scope === 'batch'} onChange={e => setScope(e.target.value)} /> Specific Batch
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                    <input type="radio" name="scope" value="time_slot" checked={scope === 'time_slot'} onChange={e => setScope(e.target.value)} /> Specific Time Slot
                  </label>
                </div>
              </div>
              ...
              {(scope === 'batch' || scope === 'time_slot') && (
                <div className="field-group">
                  <label htmlFor="batch_id" className="field-label">Select Batch *</label>
              ...
              {scope === 'time_slot' && (
                <div className="field-group form-full">
                  <label htmlFor="time_slot_id" className="field-label">Select Time Slot *</label>
      ...
      <div className="card">
        <div className="card-header"><div className="card-title">Upcoming & Past Leaves</div></div>
        <div className="table-wrap">
          <table className="data-table" aria-label="Leaves list">
            <thead>
              <tr>
                <th>Date</th><th>Scope</th><th>Details</th><th>Reason</th><th>Added By</th><th style={{ width: '60px' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {leaves.map(l => (
                <tr key={l.id}>
                  <td style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    <Calendar size={14} className="muted" /> {new Date(l.date).toLocaleDateString()}
                  </td>
                  <td><span className={`scope-badge scope-badge--${l.scope}`}>{l.scope.replace('_', ' ')}</span></td>
                  <td className="muted">
              ...
              {leaves.length === 0 && (
                <tr>
                  <td colSpan={6}><div className="empty-state"><span className="empty-state-icon">🌴</span><p>No leave days recorded.</p></div></td>
                </tr>
              )}
            </tbody>
          </table>
