      ...
      {batches.map(batch => (
        <div key={batch.id} className="batch-row">
          <div>
            ...
            <div className="batch-meta">Coach: {batch.coach_name}</div>
            ...
            <div className="batch-slots">
              {batch.slots.map(slot => (
                <div key={slot.id} className="slot-item">
                  ...
                  <div className="slot-actions">
                    <Link 
                      href={`/admin/batches/${batch.id}/slots/${slot.id}/edit`}
              ...
              {batch.slots.length === 0 && (
                <div style={{ fontSize: '0.8125rem', color: 'var(--a-text-muted)', padding: '0.5rem 0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end' }}>
            <Link href={`/admin/batches/${batch.id}/slots/new`} className="btn btn--secondary btn--sm">
              <Plus size={13} /> Add Slot
            </Link>
            <div style={{ display: 'flex', gap: '0.25rem', marginTop: '0.5rem' }}>
              <Link href={`/admin/batches/${batch.id}/edit`} className="btn btn--ghost btn--icon-sm" title="Edit Batch">
                <Pencil size={14} />
              </Link>
              <button 
                className="btn btn--ghost btn--icon-sm" 
